...
clean:
	...
	-rm -f ietf-netconf-quic@202?-??-??.yang
	-rm -f refs/tree-*.txt
...
$(next).xml: $(draft).xml
	...
	sed -e"s/$(basename $<)-latest/$(basename $@)/" -e"s/YYYY-MM-DD/$(shell date +%Y-%m-%d)/" ietf-netconf-quic.yang > ietf-netconf-quic\@$(shell date +%Y-%m-%d).yang
